<template>
  <div :class="[prefixCls + '-wrap']" ref="slider" @click.self="sliderClick">
    ...
    <div :class="[prefixCls + '-button-wrap']" :style="{left: maxPosition + '%'}" v-show="range" @touchstart="onHandStart($event, 'max')" @mousedown="onHandStart($event, 'max')">
  ...
  export default {
    ...
    methods: {
      ...
      sliderClick (event) {
        ...
        // 新值
        let newVal = ((currentX - sliderOffsetLeft) / this.sliderWidth * this.valueRange) + this.min;
